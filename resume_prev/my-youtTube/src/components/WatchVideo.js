...
import { YOUTUBE_VIDEO_WATCH_API,  YOUTUBE_DATA_API } from '../utils/constants';
...
const WatchVideo = () => {
    ...
    const getVideoDetails = async () => {
        const data = await Promise.all([fetch(YOUTUBE_VIDEO_WATCH_API + videoId), fetch(YOUTUBE_DATA_API)]);
    ...
    return (
        <div className={`${!isMenuOpen ? 'px-10 ' : 'px-3 backdrop-blur-sm bg-white'} col-span-10 pt-6 flex w-full mt-[40px] `}>
